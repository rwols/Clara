...
clang::CompilerInvocation *Session::makeInvocation() const {
  ...
  frontendOpts.Inputs.emplace_back(
      getFilename(), FrontendOptions::getInputKindForExtension(getFilename()));
...
void Session::resetDiagnosticsEngine() {
  ...
  mDiags =
      new DiagnosticsEngine(&mDiagIds, mDiagOpts.get(), &mDiagConsumer, false);
...
void Session::report(const char *message) const {
  ...
  if (mOptions.logCallback.is_none())
    return;
