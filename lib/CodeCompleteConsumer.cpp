...
CodeCompleteConsumer::ProcessCodeCompleteResult(
    clang::Sema &sema, clang::CodeCompletionContext context,
  ...
  for (const auto &chunk : ccs) {
    ...
    case CodeCompletionString::CK_Optional:
      ...
      if (includeOptionalArguments) {
        ProcessCodeCompleteString(*chunk.Optional, argCount, first, second,
                                  informative);
      }
      break;
...
void CodeCompleteConsumer::moveResult(
    std::vector<std::pair<std::string, std::string>> &result) {
  result = std::move(mResultList);
  mResultList.clear();
}

void CodeCompleteConsumer::clearResult() { mResultList.clear(); }
