...
void DiagnosticConsumer::BeginSourceFile(const clang::LangOptions &options,
...
void DiagnosticConsumer::HandleDiagnostic(clang::DiagnosticsEngine::Level level,
...
clang::PresumedLoc
DiagnosticConsumer::makePresumedLoc(const clang::Diagnostic &info) const
{
    if (mSourceMgr)
    {
...
void DiagnosticConsumer::doCallback(const char *severity,
                                    const clang::PresumedLoc &presumedLoc,
                                    const char *message)
{
    if (presumedLoc.isValid())
    {
        doCallback(presumedLoc.getFilename(), severity, presumedLoc.getLine(),
                   presumedLoc.getColumn(), message);
    }
    else
    {
...
void DiagnosticConsumer::doCallback(const char *filename, const char *severity,
                                    int row, int column, const char *message)
{
